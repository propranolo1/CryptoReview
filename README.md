...
在“同步聪明钱”中粘贴 `/smart-money/profile/{topTraderId}` 主页链接，或进入已有聪明钱用户后点击“登录并同步聪明钱”。软件会先读取公开主页身份，再打开独立的 Binance 登录窗口；请在窗口中自行完成登录，回到目标主页后关闭窗口，软件会继续读取主页分别开放共享的 U 本位当前仓位和最近 30 天操作。若主页只共享当前仓位，软件会把该仓位与关联公开带单成交一起核对，不再请求未共享的最新操作。该窗口使用仅存于内存的隔离会话，不复用主浏览器登录，不向应用页面暴露 Cookie，退出软件后会话即失效。
