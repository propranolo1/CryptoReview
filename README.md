...
- 支持 Binance 公开带单与聪明钱主页，两者都会按主页身份自动创建或复用独立复盘用户，不写入“我的账户”。聪明钱主页在桌面版通过隔离的网页登录会话读取最近 30 天 U 本位操作；登录信息仅在本次运行有效，不进入数据库或页面状态。
- 支持多仓、空仓、手续费和分批平仓。
...
在“同步公开带单”或“同步聪明钱”中粘贴 `/smart-money/profile/{topTraderId}` 主页链接。软件会先读取公开主页身份，再打开独立的 Binance 登录窗口；请在窗口中自行完成登录，回到目标主页后关闭窗口，软件会继续分页读取页面提供的 U 本位当前仓位和最近 30 天操作。该窗口使用仅存于内存的隔离会话，不复用主浏览器登录，不向应用页面暴露 Cookie，退出软件后会话即失效。
