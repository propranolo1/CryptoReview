...
在“同步公开带单”或“同步聪明钱”中粘贴 `/smart-money/profile/{topTraderId}` 主页链接。软件会先读取公开主页身份，再打开独立的 Binance 登录窗口；请在窗口中自行完成登录，回到目标主页后关闭窗口，软件会继续分页读取页面提供的最近 30 天 U 本位操作。该窗口使用仅存于内存的隔离会话，不复用主浏览器登录，不向应用页面暴露 Cookie，退出软件后会话即失效。
