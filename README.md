...
使用 API 自动导入交易，生成完整开平仓复盘。支持多时间框架回放交易，训练模式。
...
- 支持 Binance 公开带单与聪明钱主页，两者都会按主页身份自动创建或复用独立复盘用户，不写入“我的账户”。聪明钱主页在桌面版通过隔离的网页登录会话分别读取主页已共享的当前 U 本位仓位和最近 30 天操作；登录信息仅在本次运行有效，不进入数据库或页面状态。
- 支持多仓、空仓、手续费和分批平仓。
- 通过同源服务端代理读取 Binance Spot 与 USDⓈ-M Futures 公共历史 K 线。
- 桌面版可连接 Binance USER_DATA 只读 API，自动发现账户交易对并分别同步基础委托和新版 Algo 条件单；支持 `BTCUSDT`、`牛来USDT` 等官方英文或中文 U 本位交易对，密钥由当前操作系统安全存储加密，不进入网页或明文数据库。
...
- 左侧可按 UTC+8 最终平仓日期筛选，交易只在最后平仓日出现一次；“全部”下面提供“星标”筛选。每笔交易都可通过右键小菜单星标或取消星标，导入或同步生成的复盘也可在同一菜单确认删除并清理其专属原始订单；星标随本机复盘保存，并在交易所重新同步后保留。选择“全部”时交易列表在侧栏内部滚动，不会拉长整页，侧栏也可点击收起和恢复。顶部可切换到独立的“交易表现”模块，查看累计盈利曲线、每日盈利、胜率和平均盈亏比。
...
同步时只需选择日期范围，不再手动填写交易对。应用会从指定范围的收益流水、全账户当前基础/Algo 挂单和本机已有档案自动发现交易对，再按 Binance 的要求逐交易对查询历史；交易对校验支持官方使用的中文 Unicode 字母，同时仍拒绝 `&`、`=` 等查询参数字符。历史区间会拆成不足 7 天的窗口；单个窗口若达到 1000 条上限，还会继续按时间细分，HTTP 429 会按官方 `Retry-After` 等待后重试。结果会与 CSV/OCR 订单稳定合并，并按 `positionSide` 分开还原双向持仓。订单、当前未平仓仓位快照与复盘全部写入 SQLite 后才提示更新成功，下一次启动会直接恢复。官方普通单和条件单历史最多约 90 天；撤销或过期且零成交的旧单可能只保留 3 天，因此应定期同步并保留本地数据库。

### Binance 公开带单同步

在“同步公开带单”中粘贴 Binance `/copy-trading/lead-details/{portfolioId}` 主页链接。首次成功同步会按主页 ID 自动创建独立复盘用户并切换过去；同一主页再次同步只更新原用户。软件可按 30 秒、1 分钟或 5 分钟在本机自动读取最新成交与当前仓位，不会把带单记录写入“我的账户”。

公开仓位快照只用于核对未平仓数量与提示仓位变化，缺少成交价的快照差分不会被伪造成交易。公开记录不提供手续费，因此复盘会明确标记手续费未知。

### Binance 聪明钱主页同步（仅桌面版）

在“同步聪明钱”中粘贴 `/smart-money/profile/{topTraderId}` 主页链接，或进入已有聪明钱用户后点击“登录并同步聪明钱”。软件会先读取公开主页身份，再打开独立的 Binance 登录窗口；请在窗口中自行完成登录，回到目标主页后软件会自动读取主页分别开放共享的 U 本位当前仓位和最近 30 天操作，读取成功后自动关闭窗口并继续生成复盘，无需手动关窗。若主页只共享当前仓位，软件会把该仓位与关联公开带单成交一起核对，不再请求未共享的最新操作。该窗口使用仅存于内存的隔离会话，不复用主浏览器登录，不向应用页面暴露 Cookie，退出软件后会话即失效。

最近操作记录不包含手续费，且缺少 30 天以前的开仓时无法可靠形成完整开平仓。遇到这种情况，原始操作仍会保存，但软件只提示记录不完整，不会猜测入场或未平仓状态。此能力依赖 Binance 网页当前使用的接口，Binance 改版、地区限制或关闭分享后可能需要重新适配。
...
- Binance 单次最多返回 1000 根 K 线；应用会向前分页，常规回放最多保留约 4000 根，跨度更长时细周期仍可能无法覆盖最终离场点。
