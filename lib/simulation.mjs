...
export function mergeDefaultAndImportedTrades(defaultTrades, savedTrades) {
  const savedById = new Map(
    (Array.isArray(savedTrades) ? savedTrades : [])
      .filter((trade) => trade && typeof trade.id === "string")
      .map((trade) => [trade.id, trade]),
  );
  const refreshedDefaults = defaultTrades.map((trade) =>
    savedById.get(trade.id)?.starred === true
      ? { ...trade, starred: true }
      : trade,
  );
  const importedTrades = Array.isArray(savedTrades)
    ...
    : [];
  return [...refreshedDefaults, ...importedTrades];
}
