...
export function reconstructBinanceUsdmReplays(inputOrders, options = {}) {
  ...
  for (const orders of groups.values()) {
    ...
    let candidate = null;
    const unmatchedCloseOrderIds = new Set();
      ...
      while (remainingExecution > QUANTITY_EPSILON) {
        if (!candidate) {
          if (isClosingOrder(order)) {
            unmatchedCloseOrderIds.add(order.orderId);
    ...
    if (unmatchedCloseOrderIds.size > 0) {
      warnings.push({
        code: "missing_entry_history",
        symbol: allOrders[0].symbol,
        orderIds: [...unmatchedCloseOrderIds],
        message: `${allOrders[0].symbol} 的部分平仓缺少对应开仓历史，已保留订单，请完整同步补齐；未将平仓误当成反向开仓。`,
      });
    }
  ...
  for (const incoming of incomingByKey.values()) {
    ...
    const preservedNotes =
      existing && typeof existing.notes === "string" && existing.notes.trim() !== ""
        ? existing.notes
        : incoming.notes;
    const preservedStarred = existing?.starred === true || incoming.starred === true;
...
function replaysReferToSameSource(left, right) {
  ...
  // 补齐缺失历史后，旧复盘的“入场单”可能实际是完整交易的平仓单。
  // 只在同一自动复盘账户命名空间内，用完整来源证据替换旧的错误分段。
  if (replayWasAbsorbedByReconstruction(left, right)) return true;
...
function replayWasAbsorbedByReconstruction(previous, incoming) {
  if (!replayProvider(previous) || replayProvider(previous) !== replayProvider(incoming)) return false;
  const previousEntry = String(previous.sourceEntryOrderId ?? "");
  const incomingEntry = String(incoming.sourceEntryOrderId ?? "");
  const previousKey = String(previous.sourceKey ?? "");
  const incomingKey = String(incoming.sourceKey ?? "");
  if (!previousEntry || !incomingEntry || previousEntry === incomingEntry) return false;
  if (!previousKey.endsWith(`:${previousEntry}`) || !incomingKey.endsWith(`:${incomingEntry}`)) return false;
  if (previousKey.slice(0, -previousEntry.length) !== incomingKey.slice(0, -incomingEntry.length)) return false;
  return normalizeArray(incoming.sourceOrderIds).includes(previousEntry);
}
