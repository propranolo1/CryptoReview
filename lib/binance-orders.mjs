...
function canonicalizeReconstructionOrders(inputOrders) {
  ...
  for (const order of inputOrders) {
    ...
    const duplicateIndex = officialOrders.findIndex((existing) =>
      officialOrdersAreEquivalent(existing, order),
    );
    if (duplicateIndex < 0) {
...
function finalizeCandidate(candidate, allOrders, { openPosition = null, syncedAt = null } = {}) {
  ...
  const isOpen = Boolean(openPosition);
  ...
  return {
    ...
    ...(isOpen
      ...
      : {}),
    reconstructionNotice: `${isSmartMoney || isPublicCopyTrade
