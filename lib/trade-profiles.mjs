...
export const CHART_INDICATOR_KEYS = Object.freeze([
  "xinMentorship",
  "ema21",
  "ema200",
  "volumeColoring",
...
export const DEFAULT_INDICATOR_PANE_ORDER = Object.freeze([
  "xinMentorship",
  "volume",
  "openInterest",
  "delta",
...
const CHART_INDICATOR_KEY_SET = new Set(CHART_INDICATOR_KEYS);
const INDICATOR_PANE_KEY_SET = new Set(DEFAULT_INDICATOR_PANE_ORDER);
...
/** 内置档案会在启动时恢复，因此只允许删除用户自己创建的档案。 */
export function isProtectedTradeProfile(profileId) {
  return PROTECTED_PROFILE_IDS.has(requiredProfileId(profileId));
}

export function removeTradeProfile(existingProfiles, profileId) {
  const target = requiredProfileId(profileId);
  if (isProtectedTradeProfile(target)) {
    throw new RangeError("内置用户不能删除");
  }
  return normalizeTradeProfiles(existingProfiles)
    .filter((profile) => profile.id !== target);
}

/**
 * 把可能已被删除或由异步回调写回的用户选择修复成现有用户。
 * 删除当前用户后优先回到“我的账户”，避免受控下拉框保留不存在的 value。
 */
export function resolveTradeProfileSelection(existingProfiles, profileId) {
  const profiles = normalizeTradeProfiles(existingProfiles);
  const requested = typeof profileId === "string" ? profileId.trim() : "";
  return profiles.find((profile) => profile.id === requested) ??
    profiles.find((profile) => profile.id === DEFAULT_TRADE_PROFILE_ID) ??
    profiles[0];
}
...
export function removeRecordsForTradeProfile(records, profileId) {
  const target = requiredProfileId(profileId);
  return (Array.isArray(records) ? records : [])
    .filter((record) => getTradeProfileId(record) !== target);
}
...
function normalizeProfile(value) {
  ...
  const chartPreferences = normalizeChartPreferences(value.chartPreferences);
  return {
    id,
    name,
...
export function normalizeChartPreferences(value) {
  ...
  const rawVisibility = value.indicatorVisibility;
  if (rawVisibility && typeof rawVisibility === "object" && !Array.isArray(rawVisibility)) {
    for (const [key, visible] of Object.entries(rawVisibility)) {
      if (CHART_INDICATOR_KEY_SET.has(key) && typeof visible === "boolean") {
        indicatorVisibility[key] = visible;
      }
    }
  }

  const indicatorPaneOrder = [];
  for (const key of Array.isArray(value.indicatorPaneOrder) ? value.indicatorPaneOrder : []) {
    if (
      INDICATOR_PANE_KEY_SET.has(key) &&
      !indicatorPaneOrder.includes(key)
    ) {
      indicatorPaneOrder.push(key);
    }
  }
  for (const key of DEFAULT_INDICATOR_PANE_ORDER) {
    if (!indicatorPaneOrder.includes(key)) indicatorPaneOrder.push(key);
  }

  return { indicatorVisibility, indicatorPaneOrder };
}
