...
/** 公开带单和聪明钱一样使用稳定的独立复盘用户，不写入“我的账户”。 */
export function copyTradeProfileId(input) {
  return `profile-copy-trade-${extractLeadPortfolioId(input)}`;
}

export function createPublicLeadTradeProfile(
  existingProfiles,
  input,
  { nickname = null, now = Date.now() } = {},
) {
  const config = normalizeCopyTradeMonitorConfig(input);
  if (!config) throw new TypeError("公开带单监控配置无效");

  const profiles = Array.isArray(existingProfiles) ? existingProfiles : [];
  const id = copyTradeProfileId(config.portfolioId);
  const existing = profiles.find((profile) => profile?.id === id);
  const createdAt = validIsoTime(existing?.createdAt) ??
    requiredIsoTime(now, "用户创建时间");
  const preferredName = optionalText(nickname, 80) ??
    optionalText(config.nickname, 80) ??
    optionalText(existing?.name, PROFILE_NAME_MAX_LENGTH) ??
    `公开带单 ${config.portfolioId.slice(-6)}`;
  const name = uniqueProfileName(
    preferredName.slice(0, PROFILE_NAME_MAX_LENGTH),
    profiles.filter((profile) => profile?.id !== id),
    config.portfolioId,
  );
  const previousMonitor = existing?.copyTradeMonitor?.portfolioId === config.portfolioId
  ...
  return {
    ...
    copyTradeMonitor: {
      ...(previousMonitor ?? {}),
      ...config,
      ...(optionalText(nickname, 80) ? { nickname: optionalText(nickname, 80) } : {}),
    },
  };
}
...
export function upsertPublicLeadTradeProfile(
  ...
  const profiles = Array.isArray(existingProfiles) ? existingProfiles : [];
  const profile = createPublicLeadTradeProfile(profiles, config, options);
...
function uniqueProfileName(baseName, existingProfiles, portfolioId) {
  const names = new Set(
    existingProfiles
  ...
  const suffix = ` · ${portfolioId.slice(-4)}`;
