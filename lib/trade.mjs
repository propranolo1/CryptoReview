...
export function calculateTradePnl(trade, currentPrice) {
  ...
  const hasFundingFee = Object.hasOwn(trade, "fundingFee");
  const fundingFee = hasFundingFee
    ? finiteNumber(trade.fundingFee, "资金费")
    : 0;
  ...
  const realizedPnl = grossRealizedPnl - exitFees - exitedTradeFee + fundingFee;
  ...
  return {
    ...
    ...(hasFundingFee ? { fundingFee: cleanNumber(fundingFee) } : {}),
  };
}
